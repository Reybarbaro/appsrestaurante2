...
function renderAdmin(dishes) {	const sel = document.getElementById('f-cat');	sel.innerHTML = CATS.map(c => `<option value="${c.id}">${c.icon} ${c.name}</option>`).join('');	 	const el = document.getElementById('admin-list');	document.getElementById('admin-count').textContent = `(${dishes.length} platillos)`;	 	if (!dishes.length) {	el.innerHTML = '<div class="empty-state"><span>✏️</span>Agrega el primer platillo usando el formulario de arriba.</div>';	return;	}	 	const byCat = {};	dishes.forEach(d => (byCat[d.cat] = byCat[d.cat] || []).push(d));	 	el.innerHTML = Object.entries(byCat).map(([catId, items]) => {	const c = CATS.find(x => x.id === catId) || CATS[5];	const rows = items.map(d => `	<div class="admin-row">
